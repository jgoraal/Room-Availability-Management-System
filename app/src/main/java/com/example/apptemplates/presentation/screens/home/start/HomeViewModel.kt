...
import com.example.apptemplates.domain.usecase.FetchReservationsUseCase
import com.example.apptemplates.domain.usecase.FetchRoomsUseCase
import com.example.apptemplates.domain.usecase.ReservationUseCase
...
class HomeViewModel(
    ...
    private fun LocalDate.convertPeriodsToMillis(): Pair<Long, Long> {
        ...
        when (timePeriod) {
            ...
            TimePeriod.WEEK -> {
                start = this.atStartOfDay()
                end = this.plusDays(7).atTime(LocalTime.MAX)
            }

            TimePeriod.MONTH -> {
                start = this.atStartOfDay()
                end =
                    this.plusWeeks(4).atTime(LocalTime.MAX)
            }

package com.example.apptemplates.utils.constant

object Constants {

    //==================== App Constants ===========================================================

    const val TAG = "Roomio"

    //==============================================================================================


    //==================== Login Menu Screen =======================================================

    //  Text
    const val WELCOME_TEXT = "Witaj!"
    const val APPEALING_TEXT = "Szukasz swojej sali?"

    //  Buttons
    const val SIGN_IN_BUTTON = "Zaloguj się"
    const val FORGOT_PASSWORD_BUTTON = "Potwierdź"
    const val SIGN_UP_BUTTON = "Utwórz konto"

    //==============================================================================================


    //==================== Sign In Screen ==========================================================

    //  Header
    const val SIGN_IN_HEADER = "Zaloguj się"

    //  TextFields
    const val EMAIL_LABEL = "E-mail"
    const val PASSWORD_LABEL = "Hasło"
    const val PASSWORD_CONFIRM_LABEL = "Powtórz hasło"

    //==============================================================================================


    //==================== Sign Up Screen ==========================================================

    //  Header
    const val SIGN_UP_HEADER = "Załóż konto"
    const val SIGN_UP_CONFIRM_HEADER = "Podaj hasło"

    //  TextFields
    const val USERNAME_LABEL = "Nazwa"

    //  Button
    const val VALIDATE_BUTTON = "Przejdź dalej"
    const val CONFIRM_BUTTON = "Potwierdź"

    //  Dialog
    const val DIALOG_TITLE = "Podsumowanie"
    const val DIALOG_TEXT = "Proszę przejrzyj swoje dane i potwierdź rejestrację"
    const val DIALOG_CONFIRM_BUTTON = "Potwierdzam"
    const val DIALOG_CANCEL_BUTTON = "Anuluj"

    //==============================================================================================
}
